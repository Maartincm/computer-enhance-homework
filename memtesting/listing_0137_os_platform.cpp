...
struct os_platform {
  b32 Initialized;
  u64 LargePageSize; // NOTE(casey): This will be 0 when large pages are not
                     // supported (which is most of the time!)
...
static u64 ReadOSPageFaultCount(void) {
  ...
  GetProcessMemoryInfo(GlobalOSPlatform.ProcessHandle,
...
static u64 GetMaxOSRandomCount(void) { return 0xffffffff; }

static b32 ReadOSRandomBytes(u64 Count, void *Dest) {
  b32 Result = false;
  if (Count < GetMaxOSRandomCount()) {
    Result = (BCryptGenRandom(0, (BYTE *)Dest, (u32)Count,
...
static u64 TryToEnableLargePages(void) {
  ...
  if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES,
    ...
    if (LookupPrivilegeValue(0, SE_LOCK_MEMORY_NAME,
...
static void InitializeOSPlatform(void) {
  if (!GlobalOSPlatform.Initialized) {
    ...
    GlobalOSPlatform.ProcessHandle =
...
static void *OSAllocate(size_t ByteCount) {
  void *Result =
      VirtualAlloc(0, ByteCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  return Result;
}

static void OSFree(size_t ByteCount, void *BaseAddress) {
  (void)ByteCount; // NOTE(casey): On Windows, you don't pass the size when
                   // deallocating
...
static u64 GetMaxOSRandomCount(void) { return SIZE_MAX; }
...
static b32 ReadOSRandomBytes(u64 Count, void *Dest) {
  ...
  // you would have to check an implementation-defined limit on the size of
  // read() and do multiple read()'s to make sure you filled the entire buffer.
...
static void InitializeOSPlatform(void) {
  if (!GlobalOSPlatform.Initialized) {
    GlobalOSPlatform.Initialized = true;
    GlobalOSPlatform.CPUTimerFreq = EstimateCPUTimerFreq();
    GlobalOSPlatform.LargePageSize = 0;
    // GlobalOSPlatform.LargePageSize = TryToEnableLargePages();
  }
}
...
static void *OSAllocate(size_t ByteCount) {
  void *Result =
      mmap(0, ByteCount, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
  if (Result == MAP_FAILED) {
    printf("mmap failed with: '%s'\n", strerror(errno));
    printf("Falling back to small pages\n");
    Result = mmap(0, ByteCount, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (Result == MAP_FAILED) {
      printf("mmap failed with: '%s'\n", strerror(errno));
    }
  }
